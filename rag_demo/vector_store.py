...
class MilvusChunkStore:
    ...
    def replace_document_chunks(self, *, kb_id: str, document_id: str, chunks: list[Chunk]) -> list[Chunk]:
        with self._lock:
            if chunks:
                self._ensure_collection(dimension=len(chunks[0].embedding))
            self.delete_document_chunks(kb_id=kb_id, document_id=document_id)
            if not chunks:
                return []
            self._client.insert(
                collection_name=self.collection_name,
                data=[self._chunk_to_row(chunk) for chunk in chunks],
            )
            self._flush()
            return chunks
    ...
    def search_chunks(
        ...
        *,
        ...
        if not query_embedding or limit < 1 or not self._has_collection():
            return []
        self._ensure_collection(dimension=len(query_embedding))
    ...
    def _has_collection(self) -> bool:
        return self._client.has_collection(collection_name=self.collection_name)

    def _query_rows(self, filter_expression: str, *, output_fields: list[str]) -> list[dict]:
        return list(
            self._client.query(
                collection_name=self.collection_name,
